...
import { OrganisationRegistrationRequest, Organisation } from '../models';
...
export class OrganisationService {
   ...
   */
  registerOrganisation(request: OrganisationRegistrationRequest): Observable<Organisation> {
    ...
    return this.http.post<Organisation>(endpoint, request).pipe(catchError(this.handleError));
  }
  ...
  getOrganisation(id: string): Observable<Organisation> {
    ...
    return this.http.get<Organisation>(endpoint).pipe(catchError(this.handleError));
  }
   ...
   * TODO: Integrate with Spring Boot backend
   */
  verifyOrganisation(id: string): Observable<Organisation> {
    const endpoint = `${API_CONFIG.BASE_URL}${API_ENDPOINTS.ORGANISATION.VERIFY(id)}`;

    console.log('TODO: Verify organisation', id);

    return this.http.put<Organisation>(endpoint, {}).pipe(catchError(this.handleError));
  }
   ...
   * TODO: Integrate with Spring Boot backend
   */
  rejectOrganisation(id: string, reason: string): Observable<Organisation> {
    const endpoint = `${API_CONFIG.BASE_URL}${API_ENDPOINTS.ORGANISATION.REJECT(id)}`;

    console.log('TODO: Reject organisation', id, reason);

    return this.http.put<Organisation>(endpoint, { reason }).pipe(catchError(this.handleError));
  }
